...
void loginserver::login_reply(const std::string& sender_id)
{
	for (auto& entity : entities)
	{
		...
		{
			boost::array<unsigned char, 24> out_buffer;
			boost::array<unsigned char, 19> h1_buffer;

			Packet h1(h1_buffer.data(), 19, 1);
			...
			h1.set_field("h1z1_packet_id", login_rep);
			...
			auto data = SOE::RC4::Parse(h1_buffer.data(), h1_buffer.size());

			Packet soe(out_buffer.data(), out_buffer.size(), 1);
